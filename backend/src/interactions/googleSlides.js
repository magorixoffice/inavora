const getResults = async (slideId, Slide, Response) => {
  try {
    ...
    console.error('Error fetching Google Slides results:', error);
    throw error;
  }
};
...
const handleResponse = async (io, socket, data, Slide, Response) => {
  try {
    ...
    console.error('Error handling Google Slides response:', error);
    throw error;
  }
};

function normalizeAnswer(answer) {
  // For Google Slides, we just store the answer as-is
  return answer;
}
